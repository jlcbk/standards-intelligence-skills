...
4. 如果有授权文本，先按稳定 locator 比较 provisions，再考虑 semantic similarity。
5. 将变化分类为 added、removed、modified、renumbered、clarified 或 unknown。
6. 识别受影响 topics、entities、controls、evidence 和 answer packets。
7. 为高风险或低置信度变化生成 review tasks。
...
Version graph 使用 `schemas/document-family.schema.json`，具体变化使用
`schemas/change-packet.schema.json`。
